                                ...
                                <table>
                                    ...
                                    <tbody>
                                        <tr v-for="(info, index) in get_paysum" v-show="setPaginate(index)" :key="index">
                                            ...
                                            <td><input type="text" name="text" size="20" placeholder="쿠폰번호 입력" v-model ="coupon_no01[index].value" style="width:100%;"></td>
                                            <td><a @click="cop_replace(info.coupon_no,index)">[교환처리]</a></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
        ...
        methods : {
            ...
            async get_search(){
                ...
                ,{headers : {
                    ...
                    this.paginate_total = Math.ceil(this.get_paysum.length/this.paginate)
                })
